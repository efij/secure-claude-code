...
INPUT="${1:-}"
PATTERN_FILE="${STALLION_HOME:-$HOME/.stallion}/config/audit-evasion-patterns.regex"
...
if printf '%s' "$INPUT" | grep -Eqi '(history[[:space:]]+-c|Clear-History|rm|del|erase|truncate|Set-Content|Clear-Content|wevtutil[[:space:]]+cl|journalctl[[:space:]]+--vacuum-time=0s)'; then
  ...
  printf '%s\n' '[stallion] blocked audit evasion behavior' >&2
  printf '%s\n' 'reason: the command clears shell history, event logs, or Stallion audit evidence' >&2
