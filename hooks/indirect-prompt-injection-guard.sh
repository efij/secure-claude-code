...
input_file="$(mktemp "${TMPDIR:-/tmp}/stallion-indirect-input.XXXXXX")"
output_file="$(mktemp "${TMPDIR:-/tmp}/stallion-indirect-output.XXXXXX")"
meta_file="$(mktemp "${TMPDIR:-/tmp}/stallion-indirect-meta.XXXXXX")"
cleanup() {
