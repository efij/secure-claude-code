...
def _destructive_path_target(tokens: list[str], payload: str) -> tuple[pathlib.Path | None, str | None]:
    for token in tokens:
        if token.startswith("-"):
            continue
        if _path_like(token):
            path = stallion_destructive_surface.normalize_path_token(token)
            return path, stallion_destructive_surface.classify_path(path)
    match = re.search(r"(?i)\bof=(?P<target>[^\s]+)", payload)
    if match:
        path = stallion_destructive_surface.normalize_path_token(match.group("target"))
        return path, stallion_destructive_surface.classify_path(path)
    return None, None
...
def _move_away_hit(payload: str) -> dict[str, Any] | None:
    ...
    if tokens[0] == "git" and len(tokens) >= 4 and tokens[1] == "mv":
        ...
        return None
    src_path = stallion_destructive_surface.normalize_path_token(src_token)
    src_surface = stallion_destructive_surface.classify_path(src_path)
    dest_lower = dest_token.lower()
    if not stallion_destructive_surface.is_critical_surface(src_surface):
        return None
...
def _truncate_or_clear_hit(payload: str) -> dict[str, Any] | None:
    ...
    if path is None:
        return None
    decision = "block" if stallion_destructive_surface.is_critical_surface(surface or "") else "prompt"
...
def _tier_two_hit(payload: str) -> dict[str, Any] | None:
    ...
    if RANSOMWARE_RE.search(payload):
        path, surface = _destructive_path_target(tokens, payload)
        if path is not None and stallion_destructive_surface.is_critical_surface(surface or ""):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect Stallion destructive-intent events")
