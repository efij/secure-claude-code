...
POLICY_TAMPER_RE = re.compile(
    r"(?i)(?:CLAUDE\.md|AGENTS\.md|\.mcp\.json|hooks/hooks\.json|\.claude-plugin/|\.codex-plugin/|\.stallion/|settings\.json|disable hooks|ignore stallion|trust tool output over local policy)"
)
...
REVIEW_BYPASS_RE = re.compile(
    r"(?i)(?:--no-verify\b|HUSKY\s*=\s*0\b|SKIP(?:_[A-Z_]+)?\s*=\s*1\b|disable hooks|ignore stallion|bypass stallion|unset STALLION_HOME)"
)
...
def state_dir(root: pathlib.Path) -> pathlib.Path:
    ...
    return _home_dir() / ".stallion" / "state"
...
def _classify_origin(candidate: str | pathlib.Path | None, root: pathlib.Path) -> str:
    ...
    return stallion_tools._classify_origin(path, root)  # type: ignore[attr-defined]
...
def assess_change(root: pathlib.Path, event: str, matcher: str, payload: str) -> dict[str, Any]:
    ...
    if identity["has_secret_access"]:
        ...
        hit = _hit(
            ...
            safer_alternative="Keep Stallion, MCP, plugin, and instruction control files outside implicit hook logic and change them only through explicit reviewed edits.",
        )
        ...
        if existing.get("trust_state") == "approved" and existing.get("content_hash") == identity["content_hash"]:
            ...
            return {
                ...
                "hit": _hit(
                    "hook-drift-guard",
                    "prompt",
                    f"Review required because {identity['surface']} changed since Stallion last saw or approved it.",
                    identity,
                    ...
                    safer_alternative=f"Run `./bin/stallion hooks diff {shlex.quote(identity['location'])}` and then `./bin/stallion hooks approve {shlex.quote(identity['location'])}` if the change is legitimate.",
                ),
            }
        else:
        ...
        hit = _hit(
            ...
            identity,
            reason="This hook-bearing surface can execute during routine developer workflows, so Stallion asks for a one-time local review before treating it as trusted.",
            safer_alternative=f"Review the hook contents and then run `./bin/stallion hooks approve {shlex.quote(identity['location'])}` if this trigger is expected.",
        )
    else:
        trust_state = "trusted"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion hook trust state")
