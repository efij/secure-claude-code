...
POLICY_BYPASS_RE = re.compile(r"(?i)(disable runwall|ignore runwall|bypass runwall|ignore local policy|disable guardrails?|safety does not apply)")
...
def assess_fileop(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if isinstance(record, dict) and record.get("trust_state") == "quarantined":
        hit = _hit(
            ...
            f"Review the source first and move it out of quarantine with `./bin/runwall knowledge trust {path}` only if it is safe.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall knowledge diff {path}` and then `./bin/runwall knowledge trust {path}` if the new source content is expected.",
        )
        trust_state = "drifted"
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall knowledge trust {path}` only after confirming this vault, RAG cache, or imported knowledge surface is expected.",
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Runwall knowledge trust")
