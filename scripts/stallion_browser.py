    ...
    for domain in identity["sensitive_domains"]:
        ...
        if identity["has_export"] or identity["has_bulk_capture"]:
            return {
                ...
                "hit": _hit(
                    ...
                    f"Review the browser task and run `./bin/stallion browser allow {domain}` only if exporting from that session is expected.",
                ),
            }
        if approval_hit:
        ...
        return {
            ...
            "hit": _hit(
                ...
                f"Run `./bin/stallion browser allow {domain}` if this domain is part of your reviewed browser automation workflow.",
            ),
        }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion browser automation trust")
