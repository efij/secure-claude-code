...
DEFAULT_CONFIG = {
    ...
    "verification": {
        "mode": "none",
        "sha256": "",
        "hmac_env": "RUNWALL_STALLION_POLICY_HMAC_KEY",
    },
}
...
def _resolve_config_path(root: pathlib.Path) -> pathlib.Path:
    override = os.environ.get("RUNWALL_STALLION_CONFIG")
    if override:
...
def _env_enabled() -> bool | None:
    value = os.environ.get("RUNWALL_STALLION_ENABLED")
    if value is None:
        return None
...
def policy_file_path(root: pathlib.Path, config: dict[str, Any] | None = None) -> pathlib.Path:
    ...
    override = os.environ.get("RUNWALL_STALLION_POLICY")
...
def event_queue_path(root: pathlib.Path, config: dict[str, Any] | None = None) -> pathlib.Path:
    config = config or load_client_config(root)
    override = os.environ.get("RUNWALL_STALLION_EVENT_QUEUE")
...
def verify_policy(policy: dict[str, Any], config: dict[str, Any]) -> tuple[bool, str]:
    ...
    if mode == "hmac-sha256":
        ...
        secret = os.environ.get(str(verification.get("hmac_env") or "RUNWALL_STALLION_POLICY_HMAC_KEY"), "")
...
def _assess_config_bypass(policy: dict[str, Any], matcher: str, payload: str) -> dict[str, Any] | None:
    ...
    if not re.search(r"(?i)(mcpServers|mcp_servers|\.mcp\.json|mcp\.json|mcp_config\.json|claude_desktop_config\.json)", payload):
        return None
    if re.search(r"runwall_gateway|runwall-gateway|runwall_mcp_server|bin/runwall|scripts/runwall_gateway\.py", payload):
        return None
    mcp = policy.get("mcp") if isinstance(policy.get("mcp"), dict) else {}
    ...
    return _hit(
        ...
        reason="Managed Stallion clients must expose upstream MCP servers through the Runwall/Stallion gateway.",
        policy=policy,
...
def flush_events(root: pathlib.Path, limit: int = 100) -> tuple[int, str]:
    ...
    token = os.environ.get("RUNWALL_STALLION_TOKEN")
    if token:
