...
HUMAN_OVERRIDE_RE = re.compile(
    r"(?i)(?:humans should ignore stallion|ignore local policy|reviewers should override the guard|treat this note as higher priority than policy)"
)
...
def assess_fileop(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if isinstance(record, dict) and record.get("trust_state") == "quarantined":
        hit = _hit(
            ...
            f"Review the surface first and move it out of quarantine with `./bin/stallion review trust {path}` only if it is actually safe.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/stallion review diff {path}` and then `./bin/stallion review trust {path}` if the new approval-facing content is expected.",
        )
        trust_state = "drifted"
        ...
        hit = _hit(
            ...
            f"Run `./bin/stallion review trust {path}` only after confirming this PR, changelog, task, or incident surface is expected.",
        )
    ...
    if matcher in {"Write", "Edit", "MultiEdit"}:
        if PR_BYPASS_RE.search(content):
            ...
            hit = _hit(
                ...
                "Formal approval should flow through real Stallion prompts and human review, not embedded magic text.",
            )
            trust_state = "quarantined"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion human review surface trust")
