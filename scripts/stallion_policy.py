...
import stallion_chain
import stallion_data
import stallion_flow
import stallion_forensics
import stallion_hooks
import stallion_exec
import stallion_ipc
import stallion_knowledge
import stallion_memory
import stallion_promotion
import stallion_release
import stallion_runtime
import stallion_tools
import stallion_services
import stallion_exposure
import stallion_retention
import stallion_browser
import stallion_agents
import stallion_apps
import stallion_auth
import stallion_destructive
import stallion_delayed_exfil
import stallion_file_destructive
import stallion_handoff
import stallion_safety
import stallion_review
import stallion_artifacts
import stallion_client

_HOOK_SHELL: str | None = None
_METADATA_PREFIX = "STALLION_JSON:"
_DECISION_PRIORITY = {
    "allow": 0,
    "assist": 1,
    "warn": 2,
...
def extract_script(root: pathlib.Path, command: str) -> pathlib.Path | None:
    patterns = [
        r"bash\s+~/.stallion/(hooks/[^\"]+?\.sh)",
        r'bash\s+"\$\{CLAUDE_PLUGIN_ROOT\}/(hooks/[^\"]+?\.sh)"',
        r'bash\s+"\$\{STALLION_HOME\}/(hooks/[^\"]+?\.sh)"',
    ]
...
def resolve_hook_shell() -> str:
    ...
    candidates: list[str] = []
    for env_name in ("STALLION_BASH", "GIT_BASH", "BASH"):
    ...
    raise SystemExit("could not locate Git Bash on Windows for Stallion hook execution")
...
def run_hook(root: pathlib.Path, profile: str, script_path: pathlib.Path, payload: str):
    env = os.environ.copy()
    env["STALLION_HOME"] = str(root)
    env["STALLION_PROFILE"] = profile
    proc = subprocess.run(
...
def audit_file_path(root: pathlib.Path) -> pathlib.Path:
    path = os.environ.get("STALLION_AUDIT_FILE") or os.environ.get(
        "STALLION_AUDIT_FILE"
    )
    if path:
        return pathlib.Path(path)
    home = os.environ.get("STALLION_HOME") or os.environ.get(
        "STALLION_HOME"
    )
    if home:
...
def write_audit_event(
    ...
    *,
    ...
    base_event = stallion_runtime.with_event_context(
        {
        ...
        context,
        default_runtime=stallion_runtime.runtime_default(root),
    )
    if extra:
        base_event.update(extra)
    event = stallion_runtime.with_event_context(
        stallion_forensics.enrich_event(base_event),
        context,
        default_runtime=base_event.get("runtime") or stallion_runtime.runtime_default(root),
    )
    path = audit_file_path(root)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps(event, separators=(",", ":")) + "\n")
    stallion_forensics.record_event(root, event)
    stallion_client.record_event(root, {**event, "event_type": "PolicyDecision"})
    return event
...
def normalize_hit(
    ...
    hit["metadata"].setdefault(
        "confidence",
        stallion_forensics.DECISION_CONFIDENCE.get(decision, 0.55),
    )
    hit["metadata"].setdefault(
        "safer_alternative",
        stallion_forensics.infer_safer_alternative(hit),
    )
    return hit
...
def _synthetic_hit(rule: dict[str, Any], decision: str, prior_active_chain_alerts: list[dict[str, Any]]) -> dict[str, Any]:
    module = "stallion-chain-escalation" if rule.get("requires_active_chain") else "stallion-context-policy"
    metadata: dict[str, Any] = {"rule_id": rule.get("id")}
    metadata["confidence"] = stallion_forensics.DECISION_CONFIDENCE.get(decision, 0.82)
    ...
    return {
        "module": module,
        "name": "Stallion Chain Escalation" if rule.get("requires_active_chain") else "Stallion Context Policy",
...
def emit_audit_records(root: pathlib.Path, result: dict[str, Any], payload: str) -> None:
    context = {
        ...
        for field in stallion_runtime.CONTEXT_FIELDS
        if result.get(field) is not None
    }
    base_event = write_audit_event(
        root,
        module=(result.get("hits") or [{}])[-1].get("module", "stallion-policy"),
    ...
    stallion_flow.observe_result(root, result, payload)
    stallion_agents.observe_result(root, result, payload)
    stallion_apps.record_action(root, result, payload)
    stallion_auth.record_action(root, result, payload)
    stallion_handoff.observe_result(root, result, payload)
    stallion_destructive.record_action(root, result, payload)
...
def evaluate(
    ...
    *,
    ...
    merged_context = stallion_runtime.merge_contexts(stallion_runtime.context_from_env(), context)
    merged_context.setdefault("profile", profile)
    event_record = stallion_runtime.with_event_context(
        {
            "ts": datetime.now(timezone.utc).isoformat(),
            "profile": profile,
            "event": event,
    ...
    agent_hit = stallion_agents.assess_context(root, merged_context)
    if agent_hit:
    ...
    if event == "PreToolUse":
        stallion_assessment = stallion_client.assess_action(root, event, matcher, payload, merged_context)
    ...
    if event == "PreToolUse" and (matcher == "Bash" or matcher.startswith("mcp__")):
        exposure_assessment = stallion_exposure.assess_command(root, matcher, payload, merged_context)
        ...
        retention_assessment = stallion_retention.assess_command(root, matcher, payload, merged_context)
    ...
    if event == "PreToolUse" and matcher == "Bash":
        flow_hit = stallion_flow.assess_preflight(root, event, matcher, payload, merged_context)
        if flow_hit:
            if _DECISION_PRIORITY[flow_hit["decision"]] > _DECISION_PRIORITY[action]:
                action = flow_hit["decision"]
            results.append(flow_hit)

        delayed_exfil_assessment = stallion_delayed_exfil.assess_action(root, event, matcher, payload, merged_context)
        ...
        tool_assessment = stallion_tools.assess_command(root, payload)
        ...
        data_assessment = stallion_data.assess_command(root, payload, merged_context)
        ...
        ipc_assessment = stallion_ipc.assess_command(root, payload, merged_context)
        ...
        service_assessment = stallion_services.assess_command(root, payload, merged_context)
        ...
        browser_assessment = stallion_browser.assess_command(root, payload, merged_context)
        ...
        exec_assessment = stallion_exec.assess_command(root, payload, merged_context)
        ...
        app_assessment = stallion_apps.assess_command(root, payload, merged_context)
        ...
        auth_assessment = stallion_auth.assess_command(root, payload, merged_context)
        ...
        release_assessment = stallion_release.assess_action(root, event, matcher, payload, merged_context)
        ...
        destructive_assessment = stallion_destructive.assess_action(root, event, matcher, payload, merged_context)
        ...
        safety_assessment = stallion_safety.assess_action(root, event, matcher, payload, merged_context)
        ...
        handoff_hit = stallion_handoff.assess_action(root, payload, merged_context)
        if handoff_hit:
        ...
        agent_action_hit = stallion_agents.assess_action(root, payload, merged_context)
        if agent_action_hit:
    ...
    if event == "PreToolUse" and matcher in {"Read", "Write", "Edit", "MultiEdit"}:
        ...
        memory_assessment = stallion_memory.assess_fileop(root, event, matcher, payload, merged_context)
        ...
        knowledge_assessment = stallion_knowledge.assess_fileop(root, event, matcher, payload, merged_context)
        ...
        review_assessment = stallion_review.assess_fileop(root, event, matcher, payload, merged_context)
        ...
        artifact_assessment = stallion_artifacts.assess_fileop(root, event, matcher, payload, merged_context)
        ...
        promotion_assessment = stallion_promotion.assess_fileop(root, event, matcher, payload, merged_context)
        ...
        destructive_assessment = stallion_file_destructive.assess_fileop(root, event, matcher, payload, merged_context)
    ...
    if event == "PreToolUse" and matcher in {"Bash", "Write", "Edit", "MultiEdit"}:
        hook_assessment = stallion_hooks.assess_change(root, event, matcher, payload)
    ...
    session_result = stallion_chain.evaluate_session(root, event_record)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate Stallion policy hooks")
    ...
    cli_context = stallion_runtime.context_from_cli_values(
        {
