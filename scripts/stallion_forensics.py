...
def _mask_string(value: str) -> str:
    masked = SECRET_VALUE_RE.sub("[stallion] masked-secret", value)
    if len(masked) > 320:
        masked = masked[:320] + "..."
    return masked
    ...
    if isinstance(value, dict):
        ...
        for nested_key, nested_value in value.items():
            if SENSITIVE_KEY_RE.search(str(nested_key)):
                masked[nested_key] = "[stallion] masked-secret"
            elif str(nested_key) == "env" and isinstance(nested_value, dict):
                masked[nested_key] = {
                    env_key: "[stallion] masked-env"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Stallion forensics utilities")
