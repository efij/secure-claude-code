...
def assess_command(root: pathlib.Path, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if ipc_equivalent and not approval:
        ...
        ipc_approval_assessment = stallion_approvals.assess_match(
            root,
            kind="ipc",
    ...
    if approval:
        ...
        hit = _hit(
            ...
            f"Run `./bin/stallion services approve {target}` only after confirming this local database or admin service is part of your expected workflow.",
        )
        ...
        if existing and existing.get("fingerprint") != fingerprint:
            ...
            hit = _hit(
                ...
                f"Run `./bin/stallion services approve {target}` only after confirming the new local service is expected.",
            )
        else:
            trust_state = "prompted"
            hit = _hit(
                ...
                f"Run `./bin/stallion services approve {target}` if this local service is expected for your workflow.",
            )
    else:
        trust_state = "trusted"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion local service trust")
