...
POLICY_SURFACE_RE = re.compile(r"(?i)(?:^|/)(?:\.mcp\.json|hooks/hooks\.json|\.claude-plugin/[^/]+\.json|\.codex-plugin/[^/]+\.json|\.stallion/(?:config|state)/[^/]+|settings\.json)$")
...
def assess_fileop(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if isinstance(record, dict) and record.get("trust_state") == "quarantined":
        hit = _hit(
            ...
            f"Review the surface first and only trust it again with `./bin/stallion promotion trust {path}` if the content is actually safe.",
        )
        ...
        if RAW_HOST_RE.search(content):
            ...
            hit = _hit(
                ...
                "Keep MCP, plugin, settings, and Stallion policy files locally reviewed instead of filling them from remote content.",
            )
            trust_state = "quarantined"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion remote content promotion trust")
