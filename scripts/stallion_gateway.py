...
INTERNAL_TOOLS = [
    {
        "name": "preflight_bash",
        "description": "Evaluate a shell command against Stallion pre-tool policy.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "command": {"type": "string"},
    ...
    {
        "name": "preflight_read",
        "description": "Evaluate a file read against Stallion pre-tool policy.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "path": {"type": "string"},
    ...
    },
    {
        ...
        "description": "Evaluate a file write or edit against Stallion pre-tool policy.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "target": {"type": "string"},
...
def mask_text(text: str, *, limit: int = 240) -> str:
    ...
    for pattern in _MASK_PATTERNS:
        masked = pattern.sub("[stallion-masked]", masked)
    return masked[:limit]
...
def request_fingerprint(
    ...
    digest.update(safe_json_dumps({
        ...
        "arguments": arguments,
        "profile": profile,
        "context": stallion_runtime.normalize_context(context),
    }).encode("utf-8"))
    return digest.hexdigest()
        ...
        "hits": [
            {
                ...
                "decision": "redact",
                "exit_code": 0,
                "output": "[stallion] redacting secret-like MCP response content",
                "metadata": {
...
def normalize_server_spec(
    ...
    if context is not None:
        ...
        context.setdefault("label", "Stallion Context")
...
def decorate_tool_with_context(tool: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    label = str(context.get("label", "Stallion Context")).strip() or "Stallion Context"
    block = f"{label}:\n{text}"
    position = context.get("position", "append")
    if description:
            ...
            "hits": [
                {
                    ...
                    "decision": "block",
                    "exit_code": 2,
                    "output": "[stallion] blocked write-style SQL on a read-only MCP tool",
                    "metadata": {
                        "reason": (
...
def fingerprint_store_path(root: pathlib.Path) -> pathlib.Path:
    override = os.environ.get("STALLION_GATEWAY_FINGERPRINT_FILE")
    if override:
        ...
        with self._lock:
            for candidate in self._events:
                ...
                if chain_id and candidate.get("chain_id") == chain_id:
                    ...
                    continue
        return {
            "schema": "stallion-incident-bundle/v1",
...
class Gateway:
    ...
    def audit_gateway_event(self, event: dict[str, Any]) -> None:
        audited = stallion_policy.write_audit_event(
            self.root,
            module=event.get("module", "stallion-gateway"),
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                if parsed.path == "/api/events":
                    return self._json({"events": stallion_forensics.query_events(gateway.root, parse_qs(parsed.query))})
                if parsed.path.startswith("/api/events/") and parsed.path != "/api/events/stream":
                    event_id = parsed.path.rsplit("/", 1)[-1]
                    event = stallion_forensics.get_event(gateway.root, event_id)
                    if event is None:
                        self.send_error(HTTPStatus.NOT_FOUND)
                        return
                ...
                if parsed.path.startswith("/api/drift/"):
                    ...
                    drift = stallion_forensics.get_drift(gateway.root, drift_id)
                    if drift is None:
                        self.send_error(HTTPStatus.NOT_FOUND)
                        return
                ...
                if parsed.path == "/api/events/stream":
                    ...
                    try:
                        for event in stallion_forensics.query_events(gateway.root, {})[-50:]:
                            self.wfile.write(
            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                if parsed.path == "/api/incidents/export":
                    ...
                    if format_name not in {"zip", "json"}:
                        ...
                        return
                    output = stallion_forensics.export_incident(gateway.root, selector, format_name=format_name)
                ...
                if resolved.get("entity_type") and resolved.get("entity_key") and resolved.get("snapshot_id"):
                    if action == "approve":
                        stallion_forensics.approve_drift(
                            gateway.root,
                        ...
                        stallion_forensics.reject_drift(
                            gateway.root,
                            str(resolved["drift_id"]) if resolved.get("drift_id") else None,
                        )
                gateway.audit_gateway_event(
                    {
                        "module": "stallion-gateway",
    ...
    def registry_allowed(self, server_id: str, spec: dict[str, Any]) -> bool:
        ...
        result = stallion_policy.evaluate(self.root, self.profile, "PreToolUse", "GatewayRegistry", payload)
            ...
            prompt = self.event_store.create_prompt(
                {
                    ...
                    "hits": [
                        {
                            "module": "stallion-gateway",
                            "name": "Stallion Gateway Identity Check",
                            "category": "mcp",
                            "decision": "prompt",
                            "exit_code": 0,
                            "output": event["reason"],
    ...
    def server_identity_allowed(self, server_id: str, spec: dict[str, Any], upstream: UpstreamSession) -> bool:
        assessment = stallion_forensics.assess_server(
            self.root,
            server_id,
            spec,
    ...
    def tool_identity_allowed(self, server_id: str, tool: dict[str, Any]) -> bool:
        assessment = stallion_forensics.assess_tool(self.root, server_id, tool)
        return self.handle_identity_assessment(
            assessment=assessment,
            server_id=server_id,
    ...
    def evaluate_tool_definition(self, server_id: str, tool: dict[str, Any]) -> dict[str, Any]:
        ...
        return stallion_policy.evaluate(
            self.root,
            self.profile,
            "PreToolUse",
    ...
    def aggregated_tools(self) -> list[dict[str, Any]]:
        ...
        for server_id, spec in sorted(self.registry_servers().items()):
            stallion_server = stallion_client.assess_mcp_server(self.root, self.profile, server_id, spec)
            ...
            for tool in upstream_tools:
                raw_name = str(tool.get("name", ""))
                stallion_tool = stallion_client.assess_mcp_tool(
                    self.root,
                    self.profile,
                    server_id,
                    raw_name,
                ...
                if raw_name in internal_names:
                    drift = stallion_forensics.build_collision(
                        self.root,
                        drift_kind="internal_name_collision",
                        decision="block",
                        server_id=server_id,
                        tool_name=raw_name,
                        owners=[server_id, "stallion-gateway"],
                    )
                    self.audit_gateway_event(
                        self.identity_event(
                            decision="block",
    ...
    def handle_internal_tool(self, name: str, args: dict[str, Any]) -> dict[str, Any]:
        ...
        context = args.get("_stallion_context") or {}
        if name == "preflight_bash":
            result = stallion_policy.evaluate(
                self.root,
                profile,
                "PreToolUse",
                "Bash",
            ...
            stallion_client.record_event(
                self.root,
                {
                    "event_type": "PromptObserved",
                    "ts": stallion_client.utc_now(),
    ...
    def redact_result(self, result: dict[str, Any], evaluation: dict[str, Any]) -> dict[str, Any]:
        ...
        replacement = "[stallion] sensitive or malicious upstream content was redacted"
        ...
        if isinstance(structured, dict):
            ...
            preserved["stallion_redacted"] = True
            preserved["stallion_redactions"] = redactions
            redacted["structuredContent"] = preserved
        else:
            redacted["structuredContent"] = {
                "stallion_redacted": True,
                "stallion_redactions": redactions,
            }
        return redacted
    ...
    def handle_upstream_tool(self, full_name: str, arguments: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        request_eval = stallion_policy.evaluate(
            self.root,
            self.profile,
            "PreToolUse",
            matcher,
        ...
        response_eval = stallion_policy.evaluate(
            self.root,
            self.profile,
            "PostToolUse",
            matcher,
            ...
            self.audit_gateway_event(
                {
                    ...
                    "tool_input": safe_json_dumps(arguments),
                    "hits": response_eval["hits"],
                    "redactions": final_result.get("structuredContent", {}).get("stallion_redactions", []),
    ...
    def serve_stdio(self) -> int:
        ...
        while True:
                ...
                write_message(
                    sys.stdout.buffer,
                    {
                        ...
                        "result": {
                            ...
                            "capabilities": {"tools": {}},
                            "serverInfo": {"name": "stallion-gateway", "version": "17.1.0"},
                        },
                    },
                )
                continue
            ...
            if method == "tools/call":
                ...
                request_context = stallion_runtime.merge_contexts(
                    stallion_runtime.context_from_env(),
                    meta.get("stallion_context") if isinstance(meta, dict) else None,
                )
                try:
                    if name in {tool["name"] for tool in INTERNAL_TOOLS}:
                        internal_args = dict(arguments)
                        internal_args["_stallion_context"] = request_context
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Stallion inline MCP gateway")
