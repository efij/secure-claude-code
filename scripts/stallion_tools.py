...
def state_dir(root: pathlib.Path) -> pathlib.Path:
    ...
    return home_dir() / ".stallion" / "state"
    ...
    if high_trust and identity.get("bare_name") and path_candidates and len(path_candidates) > 1 and launch_origin in shadow_block_origins and any(item in auto_trust_origins for item in candidate_origins[1:]):
        ...
        if existing.get("trust_state") == "approved" and existing.get("resolved_path") == resolved_path and existing.get("sha256") == identity.get("sha256"):
            ...
            if changed_wrapper and wrapper_block:
                ...
                hit = _hit(
                    "tool-drift-guard",
                    "prompt",
                    f"Review required because {alias_key} changed path, hash, or execution shape since Stallion last saw it.",
                    identity,
                    ...
                    safer_alternative=f"Run `./bin/stallion tools approve {shlex.quote(alias_key)}` after reviewing the new path and hash if the change is legitimate.",
                )
        else:
        ...
        if identity.get("inline_wrapper") and wrapper_block:
            ...
            hit = _hit(
                ...
                safer_alternative=f"Review the new executable and then run `./bin/stallion tools approve {shlex.quote(alias_key)}` if it is expected.",
            )
            ...
            hit = _hit(
                ...
                identity,
                reason="Stallion has not seen this executable before and it does not come from a clearly reviewed system or package-managed location.",
                safer_alternative=f"Review the tool and then run `./bin/stallion tools approve {shlex.quote(alias_key)}` if you want to trust it locally.",
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion tool trust state")
