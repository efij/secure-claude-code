...
MEMORY_PATH_RE = re.compile(
    r"(?i)(?:^|/)(?:memory(?:[-_](?:store|state|journal|notes|cache|context|scratch))?\.(?:md|txt|json|yaml|yml)|project-memory\.md|session-memory\.json|agent-memory\.json|context-memory\.md|team-memory\.md|memory\.md|notes-to-self\.md|scratchpad\.md|decision-log\.md)$|(?:^|/)\.(?:claude|codex|stallion|agents)/memory(?:/|$)"
)
...
POLICY_BYPASS_RE = re.compile(r"(?i)(disable stallion|ignore stallion|bypass stallion|ignore local policy|turn off sandbox|disable guardrails?|treat safety as optional)")
...
def assess_fileop(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if record and record.get("trust_state") == "quarantined":
        hit = _hit(
            ...
            f"Review the source first and move it out of quarantine with `./bin/stallion memory trust {path}` only if it is actually safe.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/stallion memory diff {path}` and then `./bin/stallion memory trust {path}` if the new memory content is expected.",
        )
        trust_state = "drifted"
        ...
        hit = _hit(
            ...
            f"Run `./bin/stallion memory trust {path}` only after confirming this is a legitimate persistent memory surface.",
        )
    ...
    if matcher in {"Write", "Edit", "MultiEdit"}:
        if REMOTE_RE.search(content) and INSTRUCTION_RE.search(content):
            ...
            hit = _hit(
                ...
                "Do not store instructions that weaken Stallion or local runtime policy inside persistent memory.",
            )
            trust_state = "quarantined"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion memory trust")
