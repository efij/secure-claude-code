...
STALLION_AUDIT_WORKFLOW = """name: Stallion Audit
  ...
  audit:
    ...
    steps:
      ...
      - uses: actions/setup-python@v5
        with:
          python-version: '3.x'
      - name: Run Stallion audit
        run: |
          ./bin/stallion audit . --format json --fail-on high --output stallion-audit.json
      - name: Upload report
        uses: actions/upload-artifact@v4
        with:
          name: stallion-audit
          path: stallion-audit.json
"""
...
class Finding:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "enableWith": f"./bin/stallion install strict",
        }
        ...
        for pattern in self.secret_patterns:
            ...
            if match and any(token in rel.lower() for token in ("claude", "mcp", "agent", "plugin", "skill", "config", "json", "toml", "md", "yaml", "yml", "env")):
                self.add_finding(
                    ...
                    runtime_effect="Stallion will redact secret-like tool output and block some exfil paths, but config secrets should be removed entirely.",
                )
                break
        ...
        if lower_name in {"settings.json", "settings.local.json", ".mcp.json", "config.toml"} or path.suffix in {".json", ".toml"}:
            ...
            if wildcard_match:
                self.add_finding(
                    ...
                    runtime_effect="Stallion can block some dangerous combinations at runtime, but static wildcard permissions should still be reduced.",
                )
            ...
            if auto_approve:
                self.add_finding(
                    ...
                    runtime_effect="Stallion can prompt or block suspicious requests, but disabling blanket auto-approval improves the baseline immediately.",
                )
            ...
            for pattern in self.mcp_source_patterns:
                ...
                if match and ("server" in lower_name or "mcp" in rel.lower()):
                    self.add_finding(
                        ...
                        runtime_effect="Stallion can suppress risky upstreams at runtime, but removing the source drift eliminates the exposure.",
                    )
                    break
        ...
        if "/hooks/" in rel or lower_name.endswith(".sh") or "hook" in lower_name:
            ...
            if danger:
                self.add_finding(
                    ...
                    runtime_effect="Stallion blocks many of these chains during execution, but hooks should not ship with them at all.",
                )
            ...
            for pattern in self.prompt_patterns:
                ...
                if match and any(token in rel.lower() for token in ("claude", "agent", "skill", "prompt", "instruction")):
                    self.add_finding(
                        ...
                        runtime_effect="Stallion detects and suppresses many instruction-override bridges at runtime, but trusted docs should remain clean.",
                    )
                    break
        ...
        if "plugin" in rel.lower() and path.suffix in {".json", ".toml", ".md"}:
            ...
            if source_swap:
                self.add_finding(
                    ...
                    runtime_effect="Stallion can flag plugin source drift at runtime and install time, but static review should still lock provenance.",
                )
...
<head>
  ...
  <title>Stallion Audit Report</title>
  <style>
...
<body>
  <main>
    <section class="hero">
      <div class="card">
        <p class="sub">Stallion audit bridges static findings back into runtime guardrails.</p>
        <h1>Stallion Audit Report</h1>
...
def render_sarif(report: dict[str, Any]) -> str:
    ...
    payload = {
        ...
        "runs": [
            {
                "tool": {"driver": {"name": "Stallion Audit", "rules": rules}},
                "results": results,
            }
        ],
    }
...
def run_init(args: argparse.Namespace) -> int:
    root = pathlib.Path(args.path).expanduser().resolve()
    stallion_dir = root / ".stallion"
    stallion_dir.mkdir(parents=True, exist_ok=True)
    (stallion_dir / "audit-baseline.json").write_text(
        json.dumps(
            {
    ...
    workflow = root / ".github" / "workflows" / "stallion-audit.yml"
    workflow.parent.mkdir(parents=True, exist_ok=True)
    if not workflow.exists() or args.force:
        workflow.write_text(STALLION_AUDIT_WORKFLOW, encoding="utf-8")
    sys.stdout.write(f"Stallion baseline created in {stallion_dir}\n")
    sys.stdout.write(f"Workflow ready at {workflow}\n")
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Stallion audit and baseline utilities")
