...
def assess_context(root: pathlib.Path, context: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        if parent and parent in isolated and actor != parent:
            return {
                ...
                "metadata": {
                    ...
                    "confidence": 0.97,
                    "safer_alternative": f"Review the parent agent context first, then clear isolation explicitly with `./bin/stallion agents unisolate {parent}` if appropriate.",
    ...
    return {
        ...
        "metadata": {
            "reason": "This agent was manually isolated, so Stallion blocks further actions until the isolation is cleared.",
            "confidence": 0.98,
            "safer_alternative": f"Run `./bin/stallion agents unisolate {actor}` only after reviewing why the agent was isolated.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion agent graph state")
