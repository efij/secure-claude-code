...
Usage:
  ...
  bootstrap.sh --archive-url https://.../stallion.tar.gz [--profile profile]
  bootstrap.sh --archive-file /path/to/stallion.tar.gz [--profile profile]
EOF
}
...
make_tempdir() {
  ...
  if tmpdir="$(mktemp -d -t stallion-bootstrap 2>/dev/null)"; then
    printf '%s\n' "$tmpdir"
    return 0
  fi
  if tmpdir="$(mktemp -d "$base/stallion-bootstrap.XXXXXX" 2>/dev/null)"; then
    printf '%s\n' "$tmpdir"
    return 0
  fi
...
if [ -z "$ARCHIVE_URL" ] && [ -z "$ARCHIVE_FILE" ]; then
  ...
  if [ -n "$VERSION" ]; then
    ARCHIVE_URL="https://github.com/$REPO/releases/download/v$VERSION/stallion-$VERSION.tar.gz"
  else
...
printf 'Installing Stallion with profile %s\n' "$PROFILE"
