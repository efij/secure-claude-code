...
def build_context(
    ...
    label = args.context_label or str(pack_context.get("label") or "Runwall Context")
    return {
...
def add_server(args: argparse.Namespace) -> int:
    ...
    pack_catalog = runwall_gateway.load_pack_catalog(root)
    ...
    runtime_hint = ""
    if args.runtime:
        runtime_hint = f"\nnext: ./bin/runwall generate-runtime-config {args.runtime} {args.profile}"
...
def list_packs(args: argparse.Namespace) -> int:
    root = pathlib.Path(args.root).resolve()
    catalog = runwall_gateway.load_pack_catalog(root)
    if args.json:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Add upstream MCP servers to the Runwall gateway config")
