...
def _fileop_surface(matcher: str, payload: str) -> tuple[pathlib.Path | None, str]:
    ...
    if not tokens:
        return None, ""
    first = tokens[0]
    path = runwall_destructive_surface.normalize_path_token(first)
...
def _decision_for_surface(surface: str) -> str:
    return "block" if runwall_destructive_surface.is_critical_surface(surface) else "prompt"
...
def assess_fileop(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if path is None:
        return {"identity": None, "hit": None}
    surface = runwall_destructive_surface.classify_path(path)
    ...
    if textlike and existing_meaningful and not content.strip():
        ...
        hit = _hit(
            ...
            identity,
            f"Blocked destructive empty overwrite of {path}." if runwall_destructive_surface.is_critical_surface(surface) else f"Review required before empty overwrite of {path}.",
            "Preserve meaningful file contents or replace the file through a reviewed migration or recovery-safe workflow.",
            0.96 if runwall_destructive_surface.is_critical_surface(surface) else 0.84,
        )
        ...
        hit = _hit(
            ...
            _decision_for_surface(surface),
            identity,
            f"Blocked destructive stub replacement of {path}." if runwall_destructive_surface.is_critical_surface(surface) else f"Review required before replacing {path} with a stub-like body.",
            "Keep critical files functional and replace them only with reviewed content, not placeholders, stubs, or no-op bodies.",
            0.95 if runwall_destructive_surface.is_critical_surface(surface) else 0.82,
        )
        ...
        hit = _hit(
            ...
            _decision_for_surface(surface),
            identity,
            f"Blocked suspicious junk or ciphertext overwrite of {path}." if runwall_destructive_surface.is_critical_surface(surface) else f"Review required before replacing {path} with opaque or ciphertext-like content.",
            ...
            0.95 if runwall_destructive_surface.is_critical_surface(surface) else 0.8,
        )
        ...
        hit = _hit(
            ...
            _decision_for_surface(surface),
            identity,
            f"Blocked foreign-format overwrite of {path}." if runwall_destructive_surface.is_critical_surface(surface) else f"Review required before writing foreign-format header content into {path}.",
            ...
            0.94 if runwall_destructive_surface.is_critical_surface(surface) else 0.8,
        )
