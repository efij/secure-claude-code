...
def root_dir() -> pathlib.Path:
    configured = os.environ.get("STALLION_HOME") or os.environ.get("STALLION_HOME")
    if configured:
...
def emit_prompt(reason: str, chain_alerts: list[dict]) -> None:
    ...
    print("[stallion] review required for context-aware runtime action", file=sys.stderr)
...
def main(argv: list[str]) -> int:
    ...
    matcher = argv[2]
    payload = argv[3]
    profile = os.environ.get("STALLION_PROFILE", "balanced")
    context = stallion_runtime.context_from_env()
    ...
    action, hits = stallion_policy._apply_context_overlay(
        root,
        matcher,
        "allow",
        [],
    ...
    if should_audit:
        ...
        stallion_policy.write_audit_event(
            root,
            module=hits[-1]["module"] if hits else "stallion-context-policy",
            decision=action,
            reason=base_reason,
            tool_input=payload,
            profile=profile,
