...
AGENT_SIDECAR_RE = re.compile(r"(?i)(?:runwall|claude|codex|openclaw).{0,40}(?:socket|sidecar|ipc|agent)")
...
def detect_ipc(payload: str) -> dict[str, Any] | None:
    ...
    for regex in (UNIX_SOCKET_RE, UNIX_CONNECT_RE):
        ...
        if match:
            ...
            elif AGENT_SIDECAR_RE.search(payload) or any(item in lowered for item in ("claude", "codex", "runwall", "openclaw")):
...
def assess_command(root: pathlib.Path, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if service_equivalent and not approval:
        ...
        service_approval_assessment = runwall_approvals.assess_match(
            root,
            kind="service",
    ...
    if approval:
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall ipc diff {target}` and re-approve only if the new helper endpoint is still expected.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall ipc approve {target}` only if this local debug helper is expected.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall ipc approve {target}` only if this local model endpoint is expected in your workflow.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall ipc approve {target}` only if this IDE helper endpoint is expected.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall ipc approve {target}` only if this sidecar endpoint is part of your expected runtime setup.",
        )
    else:
        trust_state = "prompted"
        hit = _hit(
            ...
            f"Run `./bin/runwall ipc approve {target}` only after confirming this helper endpoint is expected.",
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Runwall local IPC and helper trust")
