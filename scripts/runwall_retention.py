...
def assess_command(root: pathlib.Path, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if runwall_exposure._is_read_only(matcher, payload):
        return {"identity": None, "hit": None}
    ...
    direct_sensitive = runwall_exposure._has_direct_sensitive(root, payload)
    session_labels = _session_labels(root, context)
    if not direct_sensitive and not session_labels:
        return {"identity": None, "hit": None}

    visibility = runwall_exposure._visibility_from_payload(payload)
    target = runwall_exposure._host(payload) or "external-retention"
