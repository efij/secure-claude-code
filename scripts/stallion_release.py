...
def assess_action(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if SIGNING_BYPASS_RE.search(payload):
        ...
        hit = _hit(
            ...
            f"Run `./bin/stallion release diff {target}` and re-approve only if this publish target is still expected.",
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Stallion publish and release trust")
