...
POLICY_BYPASS_RE = re.compile(
    r"(?i)(?:disable stallion|ignore stallion|bypass stallion|unset STALLION_HOME|HUSKY\s*=\s*0|--no-verify\b|ignore local policy)"
)
...
def assess_command(root: pathlib.Path, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if FETCH_EXEC_RE.search(payload) and (INLINE_SHELL_RE.search(payload) or INLINE_PYTHON_RE.search(payload) or INLINE_NODE_RE.search(payload) or PROCESS_SUB_RE.search(payload)):
        ...
        hit = _hit(
            ...
            identity,
            "Blocked inline execution that attempts to bypass Stallion or normal review boundaries.",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect Stallion fileless and inline execution state")
