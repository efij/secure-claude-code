...
RUNWALL_STATE_PATH_RE = re.compile(r"(?i)(?:^|/)\.runwall/(?:state|config)(?:/|$)|(?:^|/)(?:state|dist)/(?:audit\.jsonl|incident|forensics|evidence)")
...
RUNWALL_STATE_WIPE_RE = re.compile(
    r"(?i)(?:rm\s+-[A-Za-z]*[rf][A-Za-z]*\s+[^\n\r]*(?:\.runwall/state|audit\.jsonl|approvals\.json|hooks\.json|tools\.json|memory\.json|knowledge\.json|services\.json|browser\.json|apps\.json)|:\s*>\s*[^\n\r]*audit\.jsonl|truncate\s+-s\s+0\s+[^\n\r]*audit\.jsonl|shred\s+[^\n\r]*(?:audit\.jsonl|approvals\.json|hooks\.json|tools\.json))"
)
...
def _surface_from_path(path: pathlib.Path) -> str | None:
    ...
    if RUNWALL_STATE_PATH_RE.search(location):
        return "runwall-state"
...
def infer_safety_identity(event: str, matcher: str, payload: str) -> tuple[pathlib.Path | None, str, dict[str, Any] | None]:
    ...
    if matcher == "Bash":
        surface = None
        target = "shell"
        if RUNWALL_STATE_WIPE_RE.search(payload):
            surface = "runwall-state"
            target = ".runwall/state"
...
def assess_action(root: pathlib.Path, event: str, matcher: str, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    checks: list[tuple[str, str, re.Pattern[str], str, str]] = [
        (
            ...
            "block",
            RUNWALL_STATE_WIPE_RE,
            f"Blocked tampering with Runwall state or audit records at {location}.",
            "Do not delete or truncate Runwall state, approvals, or audit evidence from inside the runtime.",
        ),
        (
            "audit-disable-guard",
            "block",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect Runwall safety-control trust state")
