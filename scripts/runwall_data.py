...
def assess_command(root: pathlib.Path, payload: str, context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if store_class == "browser-indexeddb":
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall data approve {target}` only if this local database export is expected.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall data diff {target}` and re-approve only if this datastore target is still expected.",
        )
        ...
        hit = _hit(
            ...
            f"Run `./bin/runwall data approve {target}` only if this interactive or admin datastore access is expected.",
        )
    else:
        trust_state = "trusted"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage Runwall local datastore trust")
